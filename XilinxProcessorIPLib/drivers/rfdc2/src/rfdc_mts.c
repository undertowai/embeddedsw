...
void dacMTS(u32 *cmdVals)
{
    ...
    DAC_Sync_Config2.Tiles = tiles;	/* Sync DAC tiles as defined by bits set */
    xil_printf("DAC_Sync_Config2.Tiles: 0x%08x\r\n", DAC_Sync_Config2.Tiles);
	xil_printf("\n=== Multi-Tile Synchronization Metal Log Report ===\r\n");
   status = XRFdc_MultiConverter_Sync(RFdcInstPtr, XRFDC_DAC_TILE, &DAC_Sync_Config2);
    		...
    		if((1<<i)&DAC_Sync_Config2.Tiles) {
                ...
                xil_printf("DAC%d: Latency(T1) = %3d, Adjusted Delay "
				 "Offset(T%d) = %3d, Marker Delay = %d \r\n", i, DAC_Sync_Config2.Latency[i],
						 factor, DAC_Sync_Config2.Offset[i],DAC_Sync_Config2.Marker_Delay);
                xil_printf("=== MTS DAC Tile%d PLL Report ===\r\n",i);
                xil_printf("    DAC%d: PLL DTC Code =%d \n\r", i, DAC_Sync_Config2.DTC_Set_PLL.DTC_Code[i]);
                xil_printf("    DAC%d: PLL Num Windows =%d \n\r", i, DAC_Sync_Config2.DTC_Set_PLL.Num_Windows[i]);
                xil_printf("    DAC%d: PLL Max Gap =%d \n\r", i, DAC_Sync_Config2.DTC_Set_PLL.Max_Gap[i]);
                xil_printf("    DAC%d: PLL Min Gap =%d \n\r", i, DAC_Sync_Config2.DTC_Set_PLL.Min_Gap[i]);
                xil_printf("    DAC%d: PLL Max Overlap =%d \n\r", i, DAC_Sync_Config2.DTC_Set_PLL.Max_Overlap[i]);
                xil_printf("=== MTS DAC Tile%d T1 Report ===\r\n",i);
                xil_printf("    DAC%d: T1 DTC Code =%d \n\r", i, DAC_Sync_Config2.DTC_Set_T1.DTC_Code[i]);
                xil_printf("    DAC%d: T1 Num Windows =%d \n\r", i, DAC_Sync_Config2.DTC_Set_T1.Num_Windows[i]);
                xil_printf("    DAC%d: T1 Max Gap =%d \n\r", i, DAC_Sync_Config2.DTC_Set_T1.Max_Gap[i]);
                xil_printf("    DAC%d: T1 Min Gap =%d \n\r", i, DAC_Sync_Config2.DTC_Set_T1.Min_Gap[i]);
                xil_printf("    DAC%d: T1 Max Overlap =%d \n\n\r", i, DAC_Sync_Config2.DTC_Set_T1.Max_Overlap[i]);
	...
	tcpSendStatus(status != XRFDC_MTS_OK);
    return;
}
...
void dacMTSwl(u32 *cmdVals)
{
    ...
    XRFdc_MultiConverter_Init (&DAC_Sync_Config2, 0, 0, 0);
    DAC_Sync_Config2.Tiles = tiles;	/* Sync DAC tiles as defined by bits set */
    DAC_Sync_Config2.Target_Latency = lat_test;
    xil_printf("DAC_Sync_Config2.Tiles: 0x%08x\r\n", DAC_Sync_Config2.Tiles);
	xil_printf("\n=== Multi-Tile Synchronization Metal Log Report ===\r\n");
    status = XRFdc_MultiConverter_Sync(RFdcInstPtr, XRFDC_DAC_TILE, &DAC_Sync_Config2);
    if(status != XRFDC_MTS_OK) {
...
void adcMTSwl(u32 *cmdVals)
{
	...
	ADC_Sync_Config2.Tiles = tiles;	/* Sync DAC tiles as defined by bits set */
	ADC_Sync_Config2.Target_Latency = lat_test;
	xil_printf("ADC_Sync_Config2.Tiles: 0x%08x\r\n", ADC_Sync_Config2.Tiles);
	xil_printf("\n=== Multi-Tile Synchronization Metal Log Report ===\r\n");
	status = XRFdc_MultiConverter_Sync(RFdcInstPtr, XRFDC_ADC_TILE, &ADC_Sync_Config2);
	if(status != XRFDC_MTS_OK) {
		xil_printf("XRFdc_MultiConverter_Sync() FAILED\r\n");
		return;
	}
				...
				XRFdc_GetDecimationFactor(RFdcInstPtr, i, 0, &factor);
	               xil_printf("ADC%d: Latency(T1) = %3d, Adjusted Delay "
					 "Offset(T%d) = %3d, Marker Delay = %d \r\n", i, ADC_Sync_Config2.Latency[i],
							 factor, ADC_Sync_Config2.Offset[i],ADC_Sync_Config2.Marker_Delay);
	                xil_printf("=== MTS ADC Tile%d PLL Report ===\r\n",i);
	                xil_printf("    ADC%d: PLL DTC Code =%d \n\r", i, ADC_Sync_Config2.DTC_Set_PLL.DTC_Code[i]);
	                xil_printf("    ADC%d: PLL Num Windows =%d \n\r", i, ADC_Sync_Config2.DTC_Set_PLL.Num_Windows[i]);
	                xil_printf("    ADC%d: PLL Max Gap =%d \n\r", i, ADC_Sync_Config2.DTC_Set_PLL.Max_Gap[i]);
	                xil_printf("    ADC%d: PLL Min Gap =%d \n\r", i, ADC_Sync_Config2.DTC_Set_PLL.Min_Gap[i]);
	                xil_printf("    ADC%d: PLL Max Overlap =%d \n\r", i, ADC_Sync_Config2.DTC_Set_PLL.Max_Overlap[i]);
	                xil_printf("=== MTS ADC Tile%d T1 Report ===\r\n",i);
	                xil_printf("    ADC%d: T1 DTC Code =%d \n\r", i, ADC_Sync_Config2.DTC_Set_T1.DTC_Code[i]);
	                xil_printf("    ADC%d: T1 Num Windows =%d \n\r", i, ADC_Sync_Config2.DTC_Set_T1.Num_Windows[i]);
	                xil_printf("    ADC%d: T1 Max Gap =%d \n\r", i, ADC_Sync_Config2.DTC_Set_T1.Max_Gap[i]);
	                xil_printf("    ADC%d: T1 Min Gap =%d \n\r", i, ADC_Sync_Config2.DTC_Set_T1.Min_Gap[i]);
	                xil_printf("    ADC%d: T1 Max Overlap =%d \n\n\r", i, ADC_Sync_Config2.DTC_Set_T1.Max_Overlap[i]);
			}
		}
...
void adcMTS(u32 *cmdVals)
{
	...
	// Set MTS
	XRFdc_MultiConverter_Init (&ADC_Sync_Config2, 0, 0, 0);
	ADC_Sync_Config2.Tiles = tiles;	/* Sync ADC tiles as defined by bits set */
	xil_printf("ADC_Sync_Config2.Tiles: 0x%08x\r\n", ADC_Sync_Config2.Tiles);
	xil_printf("\n=== Multi-Tile Synchronization Metal Log Report ===\r\n");
	status = XRFdc_MultiConverter_Sync(RFdcInstPtr, XRFDC_ADC_TILE, &ADC_Sync_Config2);
	if(status != XRFDC_MTS_OK) {
