...
void rfdcReady (XRFdc* RFdcInstPtr) {
	u32 Tile_Id;
	XRFdc_IPStatus ipStatus;
	u32 val;
	...
	printf("\r\n###############################################\r\n");
	printf("=== Data Converter Status Report ===\n\r");
...
void rfdcStartup (XRFdc* RFdcInstPtr) {
	...
	for ( Tile_Id=0; Tile_Id<=3; Tile_Id++) {
		if (ipStatus.DACTileStatus[Tile_Id].IsEnabled == 1) {
			...
			if(val & XRFDC_DBG_RST_CAL_MASK) {
				...
				printf("   DAC Tile%d Power-on Sequence Step: 0x%08x\r\n",Tile_Id,
						Xil_In32(RFdcInstPtr->io, 0x0000C + 0x04000 + Tile_Id * 0x4000));
			}
		}
	}
	...
	for ( Tile_Id=0; Tile_Id<=3; Tile_Id++) {
		if (ipStatus.ADCTileStatus[Tile_Id].IsEnabled == 1) {
			...
			if(val & XRFDC_DBG_RST_CAL_MASK) {
				...
				printf("   ADC Tile%d Power-on Sequence Step: 0x%08x\r\n",Tile_Id,
						Xil_In32(RFdcInstPtr->io, 0x0000C + 0x14000 + Tile_Id * 0x4000));
			}
		}
	}
...
void dacCurrent(XRFdc* RFdcInstPtr)
{
	int Status;
	XRFdc_IPStatus ipStatus;
	u32 Tile_Id;
    	...
    	if (ipStatus.DACTileStatus[Tile_Id].IsEnabled == 1) {
			for ( Block_Id=0; Block_Id<=3; Block_Id++) {
				if (XRFdc_IsDACBlockEnabled(RFdcInstPtr, Tile_Id, Block_Id) != 0U) {
					...
					if (RFdcInstPtr->RFdc_Config.IPType < XRFDC_GEN3) {
						if (Status != XST_SUCCESS) {
							printf("XRFdc_GetOutputCurr() failed for DAC Tile%d Ch%d.\r\n",Tile_Id,Block_Id);
							return;
						}
						switch(OutputCurr) {
							case XRFDC_OUTPUT_CURRENT_20MA:
								printf("   DAC Tile%d Ch%d output current is 20mA. DAC_AVTT should be 2.5V\r\n",Tile_Id,Block_Id);
								break;
							case XRFDC_OUTPUT_CURRENT_32MA:
								printf("   DAC Tile%d Ch%d output current is 32mA. DAC_AVTT should be 3.0V\r\n",Tile_Id,Block_Id);
								break;
							default:
								printf("DAC output current is not recognized.  Channel may not be enabled.\r\n");
								break;
						}
						}// gen3
						else{
							Status = XRFdc_GetDACCompMode(RFdcInstPtr, Tile_Id, Block_Id, &OutputCurr2);
							if (Status != XST_SUCCESS) {
								printf("XRFdc_GetDACCompMode() failed for DAC Tile%d Ch%d.\r\n",Tile_Id,Block_Id);
								return;
								}
								switch(OutputCurr2) {
									case 0:
										printf("   DAC Tile%d Ch%d output current mode is set to RFSoC Gen3.\r\n",Tile_Id,Block_Id);
										printf("   DAC Tile%d Ch%d output current is set to %d uA.\r\n",Tile_Id,Block_Id,OutputCurr);
										break;
									case 1:
										printf("   DAC Tile%d Ch%d output current mode is set to RFSoC Gen1/2.\r\n",Tile_Id,Block_Id);
										break;
									default:
										printf("DAC output current is not recognized.  Channel may not be enabled.\r\n");
										break;
